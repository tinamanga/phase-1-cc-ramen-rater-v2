...
describe("handleSubmit", () => {
  it("should add a new slider image when the submit button is clicked", async () => {
    ...
    const newRamen = {
      name: "Mat",
      restaurant: "Test",
      image_url: "./assets/ramen/nirvana.jpg",
      rating: "4",
      comment: "test",
    };
  ...
  it("should attach a click listener to the new element to display its details", () => {
    ...
    newImageElement.src = newRamen.image_url;
    newImageElement.alt = newRamen.name;
    ...
    // Check if the image exists before trying to add the event listener
    if (img) {
      img.addEventListener("click", (event) => {
        handleClick(newRamen, event);
      });
    } else {
      throw new Error("Image not found after append.");
    }
